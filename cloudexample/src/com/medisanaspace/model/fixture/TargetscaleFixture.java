...
public class TargetscaleFixture {
	...
	private Targetscale targetscale = null;
	private Date expectedMeasurementDate;
	private Integer expectedActivityStatus;
	private Integer expectedMood;
	private Integer expectedMealStatus;
	private String expectedNote;
	private String expectedModuleSerialId;
	private Float expectedBodyWeight;
	private Float expectedBodyFat;
	private Float expectedBmi;
	private Float expectedMuscleMass;
	private Float expectedBoneMass;
	private Float expectedBodyWater;
	private Integer expectedKcal;
	private Float expectedTargetWeight;
	private Integer expectedAthletic;
	...
	public TargetscaleFixture(final int index, final Targetscale lastTargetscale) {
		...
		this.expectedMealStatus = RandomHelper.generateLong(
		...
		if (lastTargetscale == null) {
			...
			this.expectedBodyWeight = lastTargetscale.getBodyWeight()
					+ RandomHelper.generateFloat(0, 4) - 2.0f;
			if (this.expectedBodyWeight < 60f) {
				this.expectedBodyWeight = 60f - RandomHelper
						.generateFloat(0, 2);
			} else if (this.expectedBodyWeight > 120f) {
		...
		if (lastTargetscale == null) {
			...
			this.expectedBodyFat = lastTargetscale.getBodyFat()
					+ RandomHelper.generateFloat(0, 4) - 2f;
			if (this.expectedBodyFat < 12f) {
				this.expectedBodyFat = 12f - RandomHelper.generateFloat(0, 2);
			} else if (this.expectedBodyFat > 30f) {
		...
		if (lastTargetscale == null) {
			...
			this.expectedMuscleMass = lastTargetscale.getMuscleMass()
					+ RandomHelper.generateFloat(0, 3) - 1.5f;
			if (this.expectedMuscleMass < 30f) {
				this.expectedMuscleMass = 30f - RandomHelper
						.generateFloat(0, 2);
			} else if (this.expectedMuscleMass > 44f) {
		...
		if (lastTargetscale == null) {
			...
			this.expectedBodyWater = lastTargetscale.getBodyWater()
					+ RandomHelper.generateFloat(0, 4) - 2f;
		...
		if (lastTargetscale == null) {
			this.expectedKcal = 1450 + RandomHelper.generateLong(0l, 100l);
		} else {
			this.expectedKcal = lastTargetscale.getKcal()
					+ RandomHelper.generateLong(0, 20) - 10;
			if (this.expectedKcal < 1300) {
				this.expectedKcal = 1300 - RandomHelper.generateLong(0, 10);
			} else if (this.expectedKcal > 1700) {
				this.expectedKcal = 1700 + RandomHelper.generateLong(0, 10);
			}
		}
